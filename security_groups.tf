...
resource "aws_security_group" "ecs_tasks" {
  ...
  description = "ECS Fargate tasks - allow from ALB and from each other (Cloud Map/service mesh traffic)"
  vpc_id      = aws_vpc.main.id
  ...
  ingress {
    description = "Service-to-service via Cloud Map (self-referencing)"
    from_port   = 0
    to_port     = 65535
    protocol    = "tcp"
    self        = true
